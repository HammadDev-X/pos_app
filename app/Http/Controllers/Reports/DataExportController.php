...
class DataExportController extends Controller
{
    ...
    public function export(string $type): StreamedResponse
    {
        [$filename, $headers, $rows] = match ($type) {
            'products' => ['products.csv', ['ID', 'Name', 'SKU', 'Category', 'Price', 'Purchase Price', 'Quantity', 'Status'], Product::with('category')->get()->map(fn ($p): array => [$p->id, $p->name, $p->sku, $p->category?->name, $p->price, $p->purchase_price, $p->quantity, $p->status ? 'Active' : 'Inactive'])],
            'customers' => ['customers.csv', ['ID', 'Name', 'Email', 'Phone', 'Address', 'Pending Amount'], Customer::all()->map(fn ($c): array => [$c->id, $c->full_name, $c->email, $c->phone, $c->address, $c->pending_amount])],
